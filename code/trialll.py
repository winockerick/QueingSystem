...
# Function to handle the "Next" button click or teller's ESP signal
def handle_next_button(counter_id):
    if queue:
        # Serve the next token from the queue
        next_token_data = queue.pop(0)  # Get the first token in the queue
        next_token = next_token_data["token"]

        # Update the counter with the next token
        update_counter(counter_id, next_token)

        # Update the UI label for the counter
        ...
        print(f"No tokens in the queue for Counter {counter_id}.")# Function to mark a token as returned
...
def send_sms(phone_number, message):
    try:
        # will Replace this with briq SMS
        africastalking.initialize(username="BEQS", api_key="fca_live_mYsEF4trK4HJtYz6voDrrO5663krEJewkSMnRYMY")
...
def udp_listener():
    ...
    while True:
        try:
            ...
            if len(data) >= 15:  # Minimum length for phone (10) + token (4) + type (1)
                phone = data[:10].decode().strip()  # First 10 bytes for phone number
                token = int(data[10:14].decode())   # Next 4 bytes for token number
                token_type = data[14:].decode().strip()  # Remaining bytes for token type
                ...
                queue.sort(key=lambda x: 0 if x["type"].lower() == "priority" else 1)  # Priority tokens first
                ...
                for i, q in enumerate(queue):
                    if q["token"] == token and i <= 2:  # 0=next, 1=after next, 2=3rd position
            ...
            continue  # Keep running even after errors
